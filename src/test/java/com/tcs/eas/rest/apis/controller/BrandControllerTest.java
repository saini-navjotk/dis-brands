...
class BrandControllerTest {
	...
	void testUpdateBrandByIdIsNull() throws Exception {
		...
		ProductBrandApiModel brand1 = new ProductBrandApiModel(2000, "Apple", "USA", "Test Brand");
		ProductBrandApiModel brand2 = new ProductBrandApiModel(2001, "Samsung", "Singapore", "Test Brand");
		//given(brandService.getBrandById(201)).willReturn(brand2);		
		given(brandService.updateBrandById(brand2)).willReturn(brand2);
				
		// when -  action or the behaviour that we are going test
		ResultActions response = mockMvc.perform(MockMvcRequestBuilders.put("/api/v1/dis/brands").header("transaction-id", 123).contentType(MediaType.APPLICATION_JSON)
		        						.content(jsonBrand.write(brand1).getJson()));
